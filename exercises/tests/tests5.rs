...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: We're assuming that the provided address points to a valid
    // and properly aligned u32 value that we have exclusive access to,
    // as guaranteed by the function's contract. We cast the address to a
    // raw pointer and then dereference it to modify the value.
    unsafe {
